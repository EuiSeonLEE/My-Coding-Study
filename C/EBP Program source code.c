...
int main()
{
   ...
   while(get_parameter(fd_arch, fd_para) != 1){
         ...
         char str_outfile[30] = "outputdata_";
         char str_errfile[30] = "errdata_";
         append(str_outfile, str_HLn);
         append(str_outfile, str_ETA);
         append(str_outfile, str_bias);
         strcat(str_outfile,".txt");
         append(str_errfile, str_HLn);
         append(str_errfile, str_ETA);
         append(str_errfile, str_bias);
         strcat(str_errfile,".txt");
         
         FILE* fd_out;
         FILE* fd_err;

         if ((fd_out = fopen(str_outfile, "w")) == NULL) //Error의 값을 기록할 파일
         {
            printf("can't find file \n"); // 읽지 못하면 에러
            return -1;
         }
         if ((fd_err = fopen(str_errfile, "w")) == NULL) //디테일한 Error의 값을 기록할 파일
         {
            printf("can't find file \n"); // 읽지 못하면 에러
            return -1;
         }
